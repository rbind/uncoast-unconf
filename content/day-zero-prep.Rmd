...
- R itself [[Happy-Git ref.](https://happygitwithr.com/install-r-rstudio.html)]
- git [[Happy-Git ref.](https://happygitwithr.com/install-r-rstudio.html)]

Our recommended processes for installing these are just a little-bit for Windows and macOS, so we provide a screencast for each.

### Windows

```{r echo=FALSE}
rickroll_youtube("cKk7rioLFH0") %>%
  use_align("center") %>%
  use_rounded()
```
...
1. From the RStudio IDE Console pane (the R prompt):
...
1. From the RStudio IDE Terminal pane (access to your computer itself):

```
git --version
```

If you get a response indicating that your computer knows about git, then you're OK.
